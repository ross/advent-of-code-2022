...
class Map:
    ...
    def __init__(self, grid, debug):
        ...
        # blank path
        scores = [[self.UNVISITED] * len(grid[0]) for _ in range(len(grid))]
        self.scores = scores
    ...
    def search(self):
        ...
        # find all the possible starting points 'a' or 'S'
        candidates = []
        for y, row in enumerate(self.grid):
            for x, c in enumerate(row):
                score = self.scores[y][x]
                if self.grid[y][x] in ('a', 'S') and score != self.UNVISITED:
                    candidates.append((score, y, x))
        # sort them shortest to longest
        candidates.sort()
        if self.debug:
            pprint({'candidate': candidates})

        # the first will be the shortest
        return candidates[0][0]

    # this is roughly Dijkstra's algorithm for finding the shortest path in a
    # graph adapted to the problem. in this case all the edges are equal cost
    # and whether or not 2 nodes are connected is defined by the relationship
